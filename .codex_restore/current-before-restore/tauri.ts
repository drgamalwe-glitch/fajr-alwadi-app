...
async function mockInvoke<T>(
  ...
  if (command === "sync_customer_debt_from_car_sale") {
    const buyerName = String(args.buyerName ?? "").trim();
    const phone = String(args.phone ?? "").trim();
    const paymentType = String(args.paymentType ?? "").trim();
    if (!buyerName || paymentType === "كاش") return undefined as T;

    await mockInvoke("add_partner", { name: buyerName, phone, kind: "مطلوب" });

    const carLabel = String(args.carLabel ?? "").trim() || "سيارة";
    const saleDate = String(args.saleDate ?? "").trim() || new Date().toISOString().slice(0, 10);
    const saleKey = `بيع ${carLabel}`;
    const allTx: PartnerTransaction[] = JSON.parse(localStorage.getItem("mock_partner_transactions") ?? "[]");
    const linkedSaleDebts = allTx.some((tx) =>
      tx.partner_name === buyerName &&
      tx.kind === "مطلوب" &&
      tx.type_ === "سحب" &&
      (tx.notes ?? "").includes(saleKey),
    );
    if (linkedSaleDebts) return undefined as T;

    const customerNote = phone ? ` - ${buyerName} - ${phone}` : ` - ${buyerName}`;
    const amountPaid = Number(args.amountPaid) || 0;
    const amountRemaining = Number(args.amountRemaining) || 0;
    if (amountPaid > 0) {
      await mockInvoke("add_partner_transaction", {
        ...
        amount: amountPaid,
        date: saleDate,
        notes: `دفعة أولى - ${saleKey}${customerNote}`,
      });
    }

    if (amountRemaining > 0 && paymentType === "اقساط") {
      const months = Math.max(1, Number(args.installmentMonths) || 1);
      const perMonth = Math.floor(amountRemaining / months);
      const remainder = amountRemaining - perMonth * months;
      const baseDate = String(args.firstPaymentDate ?? "").trim() || saleDate;
      for (let i = 0; i < months; i++) {
        await mockInvoke("add_partner_transaction", {
          partner_name: buyerName,
          kind: "مطلوب",
          type_: "سحب",
          amount: i === months - 1 ? perMonth + remainder : perMonth,
          date: addMonths(baseDate, i),
          notes: `قسط ${i + 1}/${months} - ${saleKey}${customerNote}`,
        });
      }
    } else if (amountRemaining > 0 && paymentType === "موعد") {
      await mockInvoke("add_partner_transaction", {
        partner_name: buyerName,
        kind: "مطلوب",
        type_: "سحب",
        amount: amountRemaining,
        date: String(args.deliveryDate ?? "").trim() || saleDate,
        notes: `مؤجل - ${saleKey}${customerNote}`,
      });
    }
