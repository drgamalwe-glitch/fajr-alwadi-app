...
| **2**   | تقرير الجاهزية                  | `/reports/FINAL_REPORT.md`                                   | **الثاني**. يوثّق التنفيذ والمخاطر ونتائج البوابات الحالية ولا يغيّر قواعد الأعمال. |
| **3**   | كود الإنتاج                     | `/src-tauri/src/legacy/` + `/src/api/tauri.ts` + React components | **الثالث**. يمثل السلوك الفعلي؛ أي تعارض مع المصدر الأعلى bug. |
| **4**   | الاختبارات                      | `legacy/tests_module.rs` + `accounting_test_support.rs` + `test/accounting/` + `scripts/test_*.py` | **الرابع**. يثبت التطابق؛ الاختبار المخالف للمصدر الأعلى يُصحح. |
...
- إن قدّمنا `reports/FINAL_REPORT.md` على `Instructions.md`، فالتدقيق يصبح المصدر الوحيد، لكن `reports/FINAL_REPORT.md` نفسه يستند إلى `Instructions.md` — لا يحق له تغيير قاعدة محاسبية بل فقط تفسيرها.
...
- ✅ `agency_transactions` تستخدم claim مركزيًا وبصمة payload.
- ✅ `apply_car_expense_changes` يستخدم token ثابتًا للدفعة وبصمة canonical لكل الإضافات والحذوفات، وعملية واحدة مشتركة.
...
- ✅ أضيفت `cars.id` و`car_sales.id` و`accounts.id` و`installments.id` و`operations.id`؛ الكتابات المالية الجديدة تحمل الروابط الرقمية المناسبة.
- ℹ️ تبقى حقول `source_id/reference_id` النصية القديمة للتوافق والعرض، لكن قرارات الملكية والتعديل والحذف لا تعتمد عليها منفردة.
...
3. **ارجع إلى `reports/FINAL_REPORT.md`**: هل وثّق التدقيق السابق هذه الحالة؟ إن نعم، اتبع تفسيره. إن لا، انتقل إلى الخطوة 4.
...
- `reports/FINAL_REPORT.md` — المصدر #2.
