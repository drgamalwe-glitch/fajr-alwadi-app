...
> هذه الوثيقة توثّق ترحيلات المخطط من v1 إلى v35 المُطبَّقة عبر `init_db()` في `src-tauri/src/lib.rs`. القاعدة الذهبية §9.1: **"لا تعدّل Migration منشورة، أضف Migration جديدة فقط"**. كل ترحيلة تُطبَّق ضمن معاملة واحدة (`BEGIN`/`COMMIT`/`ROLLBACK`)، وأي فشل يلغي كل التغييرات ويترك `db_version` عند آخر قيمة ناجحة. الإصدار الحالي للمخطط: **v35**.
...
- `tests/migration_v32_prod.rs` — اختبار تكاملي يبني fixture v30 حتمي (أصل صالح + split صالح + يتيمان 229/230 بقيود دائنة 26,025 لكل منهما) ويطبق SQL التنظيف مرتين للتحقق من idempotency.
- `scripts/test_migration_v32_orphan_cleanup.py` — نفس الاختبار بلغة Python، 10/10 assertions.
