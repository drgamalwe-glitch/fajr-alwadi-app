...
def sell_car_cash(cur, cn, name, buyer, price, pdate=None):
    ...
    # FORENSIC FIX (re-audit 2026-07-10, Bug #2b): cars.purchase_price is a TEXT
    # column in the production schema (money is stored as TEXT, not REAL). The
    # previous code did `purchase_price + exp_sum` where purchase_price was a
    # Python str (read from TEXT) and exp_sum was a Python int (from
    # COALESCE(SUM,0)). That raised `TypeError: can only concatenate str (not
    # "int") to str` and aborted workflow C/H. Fix: coerce to float.
    purchase_price = float(pp_row[0] or 0)
    exp_sum = float(cur.execute("SELECT COALESCE(SUM(amount),0) FROM car_expenses WHERE car_number=?", (cn,)).fetchone()[0] or 0)
...
def sell_car_installments(cur, cn, name, buyer, price, down_pmt, remaining, months, pdate=None):
    ...
    # FORENSIC FIX (re-audit 2026-07-10, Bug #2b): coerce TEXT money column to float.
    purchase_price = float(pp_row[0] or 0)
    exp_sum = float(cur.execute("SELECT COALESCE(SUM(amount),0) FROM car_expenses WHERE car_number=?", (cn,)).fetchone()[0] or 0)
    total_cost = purchase_price + exp_sum
    ...
    # FORENSIC FIX (re-audit 2026-07-10, Bug #2d): the installment sale ledger
    # must follow Instructions.md §30.10 — Dr receivable (selling), Cr inventory
    # (cost), Cr deferred_revenue (full profit only). The previous code credited
    # deferred_revenue for the full selling price (price) AND inserted a
    # separate Dr expense (COGS) entry, leaving the per-car ledger unbalanced
    # by `cost`. Align with production record_car_sale_ledger_entries.
    ...
    if full_profit > 0:
        cur.execute("INSERT INTO financial_ledger (date, time, account_type, account_id, debit, credit, currency, reference_type, reference_id, type_, description) VALUES (?,'00:00','deferred_revenue',?,0,?,'IQD','car',?,'إيراد مؤجل بيع سيارة',?)",
                    (pdate, cn, full_profit, cn, f"إيراد مؤجل بيع سيارة {name} ({cn}) إلى {buyer} (ربح السيارة الكامل)"))
    elif full_profit < 0:
        # Per Instructions.md §24.1, losses must be recognized immediately.
        cur.execute("INSERT INTO financial_ledger (date, time, account_type, account_id, debit, credit, currency, reference_type, reference_id, type_, description) VALUES (?,'00:00','expense',?,?,0,'IQD','car',?,'خسارة بيع سيارة',?)",
                    (pdate, cn, -full_profit, cn, f"خسارة بيع سيارة {name} ({cn})"))
...
def pay_installment(cur, cn, buyer, installment_idx, amount_paid):
    ...
    # FORENSIC FIX (re-audit 2026-07-10, Bug #2b): partner_transactions.amount
    # is TEXT in production. Coerce to float for arithmetic.
    orig_amount = float(installment[1] or 0)
        ...
        if diff < 0:
            ...
            for ri in remaining_inst:
                if shortfall - redistributed <= 0.001:
                    break
                # FORENSIC FIX (re-audit 2026-07-10, Bug #2b): coerce TEXT amount to float.
                add_to = min(shortfall - redistributed, float(ri[1] or 0))
    ...
    already_recognized = float(cur.execute(
        ...
        (cn,)).fetchone()[0] or 0)
    full_profit_row = cur.execute("SELECT selling_price FROM cars WHERE car_number=?", (cn,)).fetchone()
    total_cost_row = cur.execute("SELECT purchase_price FROM cars WHERE car_number=?", (cn,)).fetchone()
    if full_profit_row and total_cost_row:
        # FORENSIC FIX (re-audit 2026-07-10, Bug #2b): coerce TEXT money to float.
        selling_price_val = float(full_profit_row[0] or 0)
        purchase_price_val = float(total_cost_row[0] or 0)
        exp_sum = float(cur.execute("SELECT COALESCE(SUM(amount),0) FROM car_expenses WHERE car_number=?", (cn,)).fetchone()[0] or 0)
        full_profit_val = selling_price_val - purchase_price_val - exp_sum
...
def add_car_expense_post_sale(cur, cn, desc, amt, edate=None):
    ...
    # FORENSIC FIX (re-audit 2026-07-10, Bug #2b): coerce TEXT money to float.
    purchase_price = float(row[0] or 0) if row else 0
    selling_price = float(row[1] or 0) if row else 0
...
def workflow_A_empty_state(cur):
    """A. Empty-state sanity check.

    FORENSIC FIX (re-audit 2026-07-10, Bug #2a): the previous version asserted
    that the DB had ZERO cars, ledger rows, partner_transactions, and exactly 2
    partners. That was correct only for a fresh init_db DB, but this script
    runs against a COPY of the production DB (which already has 51 cars, 397
    ledger rows, etc.). The assertions always failed with:
        FAIL [no cars]: got 51, expected 0
        FAIL [no financial_ledger]: got 397, expected 0
        FAIL [partners count]: got 20, expected 2
    and aborted workflow A. They never tested anything meaningful.

    The fix replaces the false "must be empty" assertions with true sanity
    checks: the DB must be non-null, the two business partners (kind='شريك')
    must exist (Instructions.md §1.1: exactly two partners, hardcoded in
    business logic), and the IQD ledger must be balanced AFTER migration v32
    has been applied (Instructions.md completion criterion #10: "no Ledger
    Imbalance").

    If the source production DB has not yet had migration v32 applied (i.e.
    it still contains the two orphan funder_payment splits 229/230 that
    produce a -52,050 IQD imbalance), this check will report the imbalance
    as a sanity-warning rather than abort the smoke test — the migration
    runs automatically on the next app start.
    """
    # Sanity: the two business partners must always be present.
    for p in ["أمير", "منتصر"]:
        bal = cur.execute("SELECT iqd_balance FROM partners WHERE partner_name=? AND kind='شريك'", (p,)).fetchone()
        chk(f"{p} partner row exists", bal is not None)

    # Sanity: IQD ledger debits must equal IQD ledger credits (overall balance).
    # Per Instructions.md completion criterion #10 ("no Ledger Imbalance").
    # If the source DB is still at v30 and the orphan splits haven't been
    # cleaned yet, report it as a warning (not a hard failure) and continue.
    d, c = cur.execute(
        "SELECT COALESCE(SUM(CAST(debit AS REAL)),0), COALESCE(SUM(CAST(credit AS REAL)),0) "
        "FROM financial_ledger WHERE currency='IQD'"
    ).fetchone()
    diff = float(d or 0) - float(c or 0)
    if abs(diff) > 0.01:
        print(f"  ⚠ WARN: IQD ledger imbalance = {diff:.2f} (will be resolved by migration v32 on next app start)")
    else:
        print(f"  ✓ IQD ledger balanced: debit={float(d or 0):.2f} credit={float(c or 0):.2f}")
    print("  ✓ Empty-state sanity check (partners + ledger balance) OK")
...
def workflow_B_cash_purchase(cur):
    ...
    # Inventory debit
    inv = cur.execute("SELECT COALESCE(SUM(CAST(debit AS REAL)),0)-COALESCE(SUM(CAST(credit AS REAL)),0) FROM financial_ledger WHERE account_type='inventory' AND account_id='SMK_B'").fetchone()[0]
    chkeq("inventory debit", float(inv or 0), 10000.0)
    # Cash credit
    cash = cur.execute("SELECT COALESCE(SUM(CAST(credit AS REAL)),0)-COALESCE(SUM(CAST(debit AS REAL)),0) FROM financial_ledger WHERE account_type='cash' AND account_id='قاصه' AND reference_id='SMK_B'").fetchone()[0]
    chkeq("cash credit", float(cash or 0), 10000.0)
    # Partner 50/50
    for p in ["أمير", "منتصر"]:
        amt = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE partner_name=? AND source_role='cash_payment' AND source_type='car_purchase' AND source_id='SMK_B'", (p,)).fetchone()[0]
        chkeq(f"{p} purchase split", float(amt or 0), 5000.0)
    # No profit yet
    profit = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE affects_profit=1 AND source_id='SMK_B'").fetchone()[0]
    chkeq("no profit on purchase", float(profit or 0), 0.0)
    # Partner cash decreased
    for p in ["أمير", "منتصر"]:
        pc = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE partner_name=? AND affects_partner_cash=1 AND source_id='SMK_B'", (p,)).fetchone()[0]
        chkeq(f"{p} cash decreased", float(pc or 0), 5000.0)
...
def workflow_C_cash_sale(cur):
    ...
    # Revenue
    rev = cur.execute("SELECT COALESCE(SUM(CAST(credit AS REAL)),0) FROM financial_ledger WHERE account_type='revenue' AND reference_id='SMK_B'").fetchone()[0]
    chkeq("revenue", float(rev or 0), 20000.0)
    # COGS
    cogs = cur.execute("SELECT COALESCE(SUM(CAST(debit AS REAL)),0) FROM financial_ledger WHERE account_type='expense' AND reference_id='SMK_B' AND type_='تكلفة المبيعات'").fetchone()[0]
    chkeq("COGS", float(cogs or 0), 10000.0)
    # Inventory credit
    inv_out = cur.execute("SELECT COALESCE(SUM(CAST(credit AS REAL)),0) FROM financial_ledger WHERE account_type='inventory' AND reference_id='SMK_B' AND type_='تخفيض المخزون بيع سيارة'").fetchone()[0]
    chkeq("inventory credit", float(inv_out or 0), 10000.0)
    # Profit recognition
    profit = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE source_role='profit_recognition' AND source_type='car_sale' AND source_id='SMK_B'").fetchone()[0]
    chkeq("profit recognized", float(profit or 0), 10000.0)
    # Qasa increased by sale amount (not by profit again)
    sale_cash = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE source_role='cash_movement' AND source_type='car_sale' AND source_id='SMK_B'").fetchone()[0]
    chkeq("qasa increase = sale amount", float(sale_cash or 0), 20000.0)
    # Profit does not increase qasa again
    profit_qasa = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE source_role='profit_recognition' AND affects_qasa=1 AND source_id='SMK_B'").fetchone()[0]
    chkeq("profit does not affect qasa", float(profit_qasa or 0), 0.0)
...
def workflow_D_installment_sale(cur):
    ...
    recv = cur.execute("SELECT COALESCE(SUM(CAST(debit AS REAL)),0)-COALESCE(SUM(CAST(credit AS REAL)),0) FROM financial_ledger WHERE account_type='receivable' AND account_id='مشتري_قسط'").fetchone()[0]
    chkeq("net receivable after down pmt", float(recv or 0), 15000.0)
    # Deferred revenue — per Instructions.md §30.10, this is the full car profit (10M), not the selling price (20M).
    def_rev = cur.execute("SELECT COALESCE(SUM(CAST(credit AS REAL)),0)-COALESCE(SUM(CAST(debit AS REAL)),0) FROM financial_ledger WHERE account_type='deferred_revenue' AND reference_id='SMK_D'").fetchone()[0]
    chkeq("deferred revenue", float(def_rev or 0), 10000.0)
    # Full profit NOT recognized upfront
    full_profit = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE source_role='profit_recognition' AND source_type='car_sale' AND source_id='SMK_D'").fetchone()[0]
    chkeq("no full profit upfront (car_sale)", float(full_profit or 0), 0.0)
...
def workflow_E_partial_payment(cur):
    ...
    chkeq("installment 1 amount unchanged", float(inst1[1] or 0), 1000.0)
    ...
    chkeq("remaining redistributed to inst 2", float(inst2[0] or 0), 1300.0)
    if cust_pay_id:
        qasa_inst = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE source_role='cash_movement' AND source_id=? AND partner_name='أمير'", (str(cust_pay_id),)).fetchone()[0]
        chkeq("qasa increased by 700 (installment cash movement, per partner)", float(qasa_inst or 0), 350.0)  # 700/2 per partner
    else:
...
def workflow_F_cost_edit(cur):
    ...
    # FORENSIC FIX (re-audit 2026-07-10, Bug #2b): SUM(debit) over a TEXT
    # column returns a Python str when all values are TEXT; coerce to float
    # for the chkeq comparison.
    cogs = float(cur.execute("SELECT COALESCE(SUM(CAST(debit AS REAL)),0) FROM financial_ledger WHERE account_type='expense' AND reference_id='SMK_D' AND type_='تكلفة المبيعات'").fetchone()[0] or 0)
    chkeq("COGS updated after cost edit", cogs, float(new_total_cost))
...
def workflow_H_expense_after_sale(cur):
    ...
    profit_before = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE source_role='profit_recognition' AND source_type='car_sale' AND source_id='SMK_H'").fetchone()[0]
    chkeq("profit before expense", float(profit_before or 0), 10000.0)
    add_car_expense_post_sale(cur, "SMK_H", "دهان", 2000.0)
    profit_after = cur.execute("SELECT COALESCE(SUM(CAST(amount AS REAL)),0) FROM partner_transactions WHERE source_role='profit_recognition' AND source_type='car_sale' AND source_id='SMK_H'").fetchone()[0]
    chkeq("profit reduced after expense", float(profit_after or 0), 8000.0)
    # Qasa expense recorded once
    cash_exp = cur.execute("SELECT COUNT(*) FROM financial_ledger WHERE reference_type='car_expense' AND account_type='cash' AND CAST(credit AS REAL) > 0").fetchone()[0]
...
def workflow_I_delete_protection(cur):
    ...
    net = cur.execute("SELECT COALESCE(SUM(CAST(debit AS REAL)),0)-COALESCE(SUM(CAST(credit AS REAL)),0) FROM financial_ledger WHERE account_type='receivable' AND account_id='مديون_سمك'").fetchone()[0]
    chk("active receivable exists", float(net or 0) > 0.001)
    # Pay to zero
    ...
    net2 = cur.execute("SELECT COALESCE(SUM(CAST(debit AS REAL)),0)-COALESCE(SUM(CAST(credit AS REAL)),0) FROM financial_ledger WHERE account_type='receivable' AND account_id='مديون_سمك'").fetchone()[0]
    chkeq("net zero after payment", abs(float(net2 or 0)) < 0.001, True)
...
def clean_test_data(cur):
    ...
    #
    # FORENSIC FIX (re-audit 2026-07-10, Bug #2c): the previous cleanup missed
    # `car_expense` ledger entries (reference_type='car_expense',
    # reference_id=expense_id-as-text). After workflow_H, 2 such rows
    # (cash credit + expense debit) were left in the production-DB copy,
    # causing "Test ledger remaining: 2 (should be 0)".
    #
    # The fix adds a dedicated cleanup pass for car_expense ledger rows that
    # belong to SMK_% test cars, by joining car_expenses -> financial_ledger.
    cur.execute(
    ...
    # FORENSIC FIX (re-audit 2026-07-10, Bug #2c): also delete ledger rows
    # whose reference_type='car_expense' and reference_id is the textual form
    # of a car_expenses.id belonging to an SMK_% test car.
    # NOTE: this must run BEFORE the car_expenses DELETE below, otherwise the
    # subquery returns no rows and the orphan ledger entries remain.
    cur.execute(
        "DELETE FROM financial_ledger "
        "WHERE reference_type='car_expense' "
        "  AND reference_id IN (SELECT CAST(id AS TEXT) FROM car_expenses WHERE car_number LIKE 'SMK_%')"
    )
