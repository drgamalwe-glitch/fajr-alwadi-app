...
def audit_db(db_path):
    ...
    collisions = conn.execute("""
        SELECT source_type, source_id, source_role, partner_name, kind,
               COALESCE(currency, 'IQD') AS currency,
               COALESCE(related_source_id, '') AS related_source_id,
               COUNT(*) as cnt
        FROM partner_transactions
        ...
        GROUP BY source_type, source_id, source_role, partner_name, kind,
                 COALESCE(currency, 'IQD'), COALESCE(related_source_id, '')
        HAVING cnt > 1
    """).fetchall()
    for c in collisions:
        errors.append(
            f"FAIL: Duplicate source: {c['source_type']}/{c['source_id']}/{c['source_role']} "
            f"currency={c['currency']} related={c['related_source_id']} ({c['cnt']} rows)"
        )
    ...
    # ─────────────────────────────────────────────────────────────────────
    # FORENSIC FIX (re-audit 2026-07-10, Bug #3): Global Ledger Balance Check
    #
    # The previous audit had per-car ledger balance checks (audits [25] and
    # [39]) and per-account_type aggregations, but NO check that the OVERALL
    # ledger debits equal the OVERALL ledger credits per currency.
    #
    # This gap allowed a -52,050 IQD ledger imbalance in the production DB
    # (caused by two orphan `funder_payment` split rows 229/230 whose
    # `source_id=228` referenced a deleted funder_transaction parent) to go
    # undetected by the audit for the entire production-DB lifetime. The
    # per-car checks passed because the orphan ledger entries were
    # `reference_type='partner_transaction'`, not `reference_type='car'`, so
    # they were invisible to the per-car balance scope.
    #
    # Per Instructions.md completion criterion #10 ("عدم وجود Ledger
    # Imbalance"), this check is now mandatory. It runs as the final audit
    # so it can catch ANY imbalance regardless of which subsystem produced it.
    # ─────────────────────────────────────────────────────────────────────
    print("\n[80] Global ledger balance per currency (FORENSIC FIX Bug #3)...")
    currency_rows = conn.execute("""
        SELECT currency,
               COALESCE(SUM(CAST(debit AS REAL)), 0.0) AS total_debit,
               COALESCE(SUM(CAST(credit AS REAL)), 0.0) AS total_credit
        FROM financial_ledger
        GROUP BY currency
    """).fetchall()
    global_balance_ok = True
    for cr in currency_rows:
        cur_code = cr['currency']
        td = float(cr['total_debit'] or 0.0)
        tc = float(cr['total_credit'] or 0.0)
        diff = td - tc
        if abs(diff) > 0.01:
            errors.append(
                f"FAIL: Global ledger imbalance for currency {cur_code}: "
                f"debit={td:,.2f} credit={tc:,.2f} diff={diff:,.2f}"
            )
            global_balance_ok = False
    if global_balance_ok:
        print("  PASS")
    else:
        print("  FAIL")

    # ─────────────────────────────────────────────────────────────────────
    # FORENSIC FIX (re-audit 2026-07-10, Bug #3b): Orphan partner_transaction
    # split rows — splits whose source_id points to a deleted parent.
    #
    # This is the SAME class of corruption that produced the -52,050 IQD
    # imbalance in the production DB (rows 229/230 referencing missing 228).
    # The migration v32 cleanup_orphan_partner_splits() removes them at
    # migration time, but the audit must also flag any that exist NOW so
    # that future regressions of the migration logic are caught.
    # ─────────────────────────────────────────────────────────────────────
    print("\n[81] No orphan partner_transaction split rows (FORENSIC FIX Bug #3b)...")
    orphan_splits = conn.execute("""
        SELECT COUNT(*) FROM partner_transactions
    ...
    if orphan_splits > 0:
        errors.append(
            f"FAIL: {orphan_splits} orphan partner_transaction split rows "
            f"(source_id points to a deleted parent) — these rows reduce "
            f"partner cash / funder liability without the matching counterpart "
            f"and will cause a global ledger imbalance. Run migration v32 "
            f"(cleanup_orphan_partner_splits) to fix."
        )
        print(f"  FAIL ({orphan_splits} orphans)")
    else:
        print("  PASS")

    # ─────────────────────────────────────────────────────────────────────
    # FORENSIC FIX (re-audit 2026-07-10, Bug #3c): Source metadata completeness
    # per Instructions.md §31.6.
    #
    # Every partner_transaction and every financial_ledger row must have
    # non-null, non-empty source_type, source_id, and source_role. Rows
    # that do not meet this requirement are considered corrupt and cannot
    # be safely deleted or reversed (they have no source reference).
    # ─────────────────────────────────────────────────────────────────────
    print("\n[82] Source metadata completeness (FORENSIC FIX Bug #3c, §31.6)...")
    bad_pt_metadata = conn.execute("""
        SELECT COUNT(*) FROM partner_transactions
        WHERE (source_type IS NULL OR TRIM(source_type) = '')
           OR (source_id IS NULL OR TRIM(source_id) = '')
           OR (source_role IS NULL OR TRIM(source_role) = '')
    """).fetchone()[0]
    if bad_pt_metadata > 0:
        errors.append(
            f"FAIL: {bad_pt_metadata} partner_transactions rows have missing "
            f"source_type/source_id/source_role (Instructions.md §31.6 requires "
            f"all three to be non-null and non-empty)"
        )
        print(f"  FAIL ({bad_pt_metadata} rows with missing metadata)")
    else:
        print("  PASS (partner_transactions)")

    bad_fl_metadata = conn.execute("""
        SELECT COUNT(*) FROM financial_ledger
        WHERE (reference_type IS NULL OR TRIM(reference_type) = '')
           OR (reference_id IS NULL OR TRIM(reference_id) = '')
    """).fetchone()[0]
    if bad_fl_metadata > 0:
        errors.append(
            f"FAIL: {bad_fl_metadata} financial_ledger rows have missing "
            f"reference_type/reference_id"
        )
        print(f"  FAIL ({bad_fl_metadata} ledger rows with missing metadata)")
    else:
        print("  PASS (financial_ledger)")

    # ─────────────────────────────────────────────────────────────────────
    # FORENSIC FIX (re-audit 2026-07-10, Bug #3d): Agency cash vs credit
    # profit recognition per Instructions.md §31.4.
    #
    # Credit agencies (payment_status='غير واصل') must NOT have
    # profit_recognition or cash_movement rows. Only receivable rows are
    # allowed. Cash agencies (payment_status='واصل') must have both.
    # ─────────────────────────────────────────────────────────────────────
    print("\n[83] Agency cash vs credit profit recognition (§31.4)...")
    credit_agencies_with_profit = conn.execute("""
        SELECT COUNT(*) FROM partner_transactions pt
        WHERE pt.source_type = 'agency'
          AND pt.source_role IN ('profit_recognition', 'cash_movement')
          AND pt.source_id IN (
            SELECT CAST(id AS TEXT) FROM agencies
            WHERE COALESCE(payment_status, 'واصل') = 'غير واصل'
          )
          AND COALESCE(pt.is_reversed, 0) = 0
    """).fetchone()[0]
    if credit_agencies_with_profit > 0:
        errors.append(
            f"FAIL: {credit_agencies_with_profit} profit/cash rows exist for "
            f"credit agencies (payment_status='غير واصل') — Instructions.md "
            f"§31.4.2 forbids profit recognition until payment is received"
        )
        print(f"  FAIL ({credit_agencies_with_profit} violating rows)")
    else:
        print("  PASS (no profit for credit agencies)")
...
def audit_source(lib_path):
    ...
    update_start = content.find("fn update_expense(")
    update_end = content.find("\n#[tauri::command]", update_start + 10) if update_start >= 0 else -1
    update_body = content[update_start:update_end if update_end > 0 else update_start + 7000]
    if update_start >= 0 and "delete_ledger_entries" in update_body and "reverse_ledger_entries" not in update_body:
        print("  PASS")
    else:
        errors.append("FAIL: update_expense is missing clean delete-and-rebuild ledger handling")
    ...
    for func_name in ['fn add_expense', 'fn update_expense', 'fn apply_car_expense_changes']:
        in_func = False
        has_validation = False
        func_start_line = 0
        for i, line in enumerate(lines, 1):
    ...
    # 66. Atomic car-expense command rebuilds sold-car accounting safely
    print("\n[S66] Atomic car-expense command rebuilds sold-car accounting...")
    fn_start = content.find("fn apply_car_expense_changes")
    if fn_start >= 0:
        fn_end = content.find("\n#[tauri::command]", fn_start + 10)
        fn_body = content[fn_start:fn_end if fn_end > 0 else fn_start + 9000]
        has_rebuild = "rebuild_sold_car_accounting_after_cost_change" in fn_body
        has_old_api = "add_car_expense_record" in content or "delete_car_expense_record" in content
        if has_rebuild and not has_old_api:
            print("  PASS")
        else:
            errors.append("FAIL: atomic car-expense rebuild missing or legacy write APIs remain")
    else:
        errors.append("FAIL: apply_car_expense_changes not found in lib.rs")
    ...
    sold_paths_with_old = ["add_expense", "apply_car_expense_changes"]
    offending = 0
    ...
    # ─── S75: car expense edit batch must be atomic and precisely scoped ───
    print("\n[S75] Car expense changes are atomic and precisely scoped...")
    expense_fn_start = content.find("fn apply_car_expense_changes")
    expense_fn_end = content.find("\n#[tauri::command]", expense_fn_start + 10) if expense_fn_start >= 0 else -1
    if expense_fn_start >= 0:
        expense_body = content[expense_fn_start:expense_fn_end if expense_fn_end > 0 else expense_fn_start + 8000]
        expense_requirements = {
            "transaction": ".transaction()" in expense_body,
            "ownership check": "SELECT car_number FROM car_expenses WHERE id = ?1" in expense_body,
            "precise delete": "DELETE FROM car_expenses WHERE id = ?1 AND car_number = ?2" in expense_body,
            "affected-row check": "if affected != 1" in expense_body,
            "single commit": expense_body.count(".commit()") == 1,
        }
        missing = [name for name, ok in expense_requirements.items() if not ok]
        if missing:
            errors.append(f"FAIL: apply_car_expense_changes missing {', '.join(missing)}")
        else:
            print("  PASS (transaction + ownership + precise delete + row-count check)")
    else:
        errors.append("FAIL: apply_car_expense_changes command not found")

    # ─── S76: frontend must save car before expenses and keep editor open on failure ───
    print("\n[S76] Frontend car/expense save ordering and failure handling...")
    car_form_path = Path("src/components/CarFormPanel.tsx")
    if car_form_path.exists():
        car_form = car_form_path.read_text(encoding="utf-8")
        car_saved_pos = car_form.find("const carSaved = await onSubmit(e)")
        expense_save_pos = car_form.find("await saveExpenseChanges();", car_saved_pos)
        atomic_expense_command = 'await callTauri("apply_car_expense_changes"' in car_form
        closes_only_after_success = "if (!carSaved) return;" in car_form and "onSaveComplete?.();" in car_form
        old_mutation_absent = 'callTauri("add_car_expense_record"' not in car_form and 'callTauri("delete_car_expense_record"' not in car_form
        if 0 <= car_saved_pos < expense_save_pos and atomic_expense_command and closes_only_after_success and old_mutation_absent:
            print("  PASS (car first, atomic expense command second, no close on failure)")
        else:
            errors.append("FAIL: unsafe frontend car-expense save ordering or legacy mutations remain")
    else:
        errors.append("FAIL: CarFormPanel.tsx not found")

    # ─── S77: no dead duplicate sale automation in CarsTab ───
    print("\n[S77] No dead duplicate sale automation remains in CarsTab...")
    cars_tab_path = Path("src/components/CarsTab.tsx")
    if cars_tab_path.exists():
        cars_tab = cars_tab_path.read_text(encoding="utf-8")
        dead_markers = ["executeSaleAutomation", "handleSaleConfirm", "showSaleConfirm", "pendingSaleData"]
        remaining = [m for m in dead_markers if m in cars_tab]
        if remaining:
            errors.append(f"FAIL: dead duplicate sale path remains: {', '.join(remaining)}")
        else:
            print("  PASS")
    else:
        errors.append("FAIL: CarsTab.tsx not found")

    # ─── S78: write commands without explicit token still require a live session ───
    print("\n[S78] Compatibility write path requires a live unexpired admin session...")
    session_fn_start = content.find("fn require_admin_session")
    session_fn_end = content.find("\nfn ", session_fn_start + 5) if session_fn_start >= 0 else -1
    if session_fn_start >= 0:
        session_body = content[session_fn_start:session_fn_end if session_fn_end > 0 else session_fn_start + 4000]
        if "FROM sessions WHERE user_id = ?1 AND expires_at > ?2" in session_body and "active_sessions == 0" in session_body:
            print("  PASS (pre-login and expired-session writes are rejected)")
        else:
            errors.append("FAIL: require_admin_session(None) does not enforce a live session")
    else:
        errors.append("FAIL: require_admin_session not found")

    # ─── S80: financial summaries and account deletion must not hide DB errors ───
    print("\n[S80] Financial summaries and safe-delete queries surface database errors...")
    critical_functions = [
    ...
    for fn_name in critical_functions:
        fn_start = content.find(f"fn {fn_name}(")
        if fn_start < 0:
            silent_functions.append(fn_name + "(missing)")
            continue
        ...
        for pos in range(brace_start, len(content)):
            ...
            if ch == '"':
                ...
                if depth == 0:
                    fn_end = pos + 1
                    break
        fn_body = content[fn_start:fn_end if fn_end > 0 else fn_start + 12000]
        if ".query_row" in fn_body and ".unwrap_or(Money::zero())" in fn_body:
            silent_functions.append(fn_name)
    if silent_functions:
        errors.append("FAIL: financial query failures can still become silent zeroes in " + ", ".join(silent_functions))
    else:
        print("  PASS")

    # ─── S79: source tree must not retain temporary-work markers ───
    print("\n[S79] No TODO/FIXME/HACK/workaround markers in audited source...")
    marker_hits = []
    for root in [Path("src"), Path("src-tauri/src"), Path("scripts")]:
        if not root.exists():
            continue
        for file_path in root.rglob("*"):
            if not file_path.is_file() or file_path.suffix not in {".rs", ".ts", ".tsx", ".py", ".css", ".json"}:
                continue
            if file_path.resolve() == Path(__file__).resolve():
                continue
            file_text = file_path.read_text(encoding="utf-8", errors="ignore")
            for marker_word in ["TODO", "FIXME", "HACK", "WORKAROUND"]:
                if marker_word in file_text:
                    marker_hits.append(f"{file_path}:{marker_word}")
    if marker_hits:
        errors.append("FAIL: temporary-work markers remain: " + ", ".join(marker_hits[:10]))
    else:
        print("  PASS")
