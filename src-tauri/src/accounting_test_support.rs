...
impl TestHarness {
    pub fn new() -> Self {
        ...
        // Test-only authenticated actor. Production creates this user through
        // the one-time bootstrap_admin command.
        let password_hash =
            crate::hash_password("test-only-password-2026").expect("hash test admin password");
        match conn.execute(
            "INSERT INTO users
             (id, username, password_hash, display_name, must_change_password)
             VALUES (1, 'admin', ?1, 'Test Admin', 0)",
            [&password_hash],
        ) {
            Ok(_) => {}
            Err(e) => panic!("failed to clear must_change_password for test admin: {e}"),
        }
        // FORENSIC FIX (re-audit 2026-07-11, TEST-SAFETY-NET-1):
        // The accounting_real_backend_full_71 suite exercises Tauri commands that
        // gate on `require_admin_session`. Without a live session row for user_id=1,
        // every command silently returns "no active session" and the suite passes
        // vacuously. Insert a long-lived session row so the gate is satisfied for
        // the lifetime of the test process. NEVER do this in production code.
        let now = chrono::Utc::now().timestamp();
        let far_future = now + 86_400;
        match conn.execute(
            "INSERT OR REPLACE INTO sessions (token, user_id, created_at, expires_at)
             VALUES (?1, 1, ?2, ?3)",
            rusqlite::params!["test-session-token", now, far_future],
        ) {
            Ok(_) => {}
            Err(e) => panic!("failed to seed test session for user_id=1: {e}"),
        }
