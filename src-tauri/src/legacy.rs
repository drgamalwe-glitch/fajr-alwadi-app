...
pub fn ensure_installment_event_schema(conn: &Connection) -> SqlResult<()> {
    ...
    // Previously every ALTER used `let _ = ...` which silently swallowed ALL
...
// FORENSIC FIX (re-audit 2026-07-11, SECURITY-1):
// Hardcoded `admin/admin` default and the `initial_admin_password.txt` artifact
// have been removed. The primary admin is now bootstrapped with a cryptographically
// random one-time password generated via OsRng. The plaintext is shown to the
// operator EXACTLY ONCE on stderr (so installers can capture it) and is never
// written to disk in any form. The user record is created with
// `must_change_password = 1`, which `require_admin_session` enforces as a hard
// gate before any write command is allowed.
//
// Backward compatibility: existing databases that already have a primary admin
// row are left UNTOUCHED — we never reset an existing password. Databases that
// still carry the legacy `admin/admin` hash from a previous install are flagged
// in the log so the operator can rotate them; we do NOT silently revert them.
pub const LEGACY_INSECURE_ADMIN_PASSWORD: &str = "admin";

pub fn hash_password_for_init(password: &str) -> SqlResult<String> {
...
pub fn generate_one_time_admin_password() -> String {
...
pub fn is_legacy_insecure_password(plaintext: &str) -> bool {
    plaintext == LEGACY_INSECURE_ADMIN_PASSWORD
}
...
pub fn cleanup_expired_sessions(conn: &Connection) {
    ...
    // Previously this used `let _ = ...` which silently swallowed errors.
...
pub fn init_db(conn: &Connection) -> SqlResult<()> {
    ...
    let init_result: SqlResult<()> = (|| {
        ...
        // FORENSIC FIX (re-audit 2026-07-11, SECURITY-1):
        // Secure bootstrap policy — see §7.1 of the executive prompt.
        //
        // Case A — Fresh install (no primary admin row):
        //   Leave users empty. The UI calls `bootstrap_admin`, which requires
        //   the operator to choose the first password and is transactionally
        //   disabled forever after the first user is created.
        //
        // Case B — Existing install with must_change_password = 1:
        //   Leave the existing hash in place. The must_change flag stays on so
        //   the operator must still rotate on next login. We do NOT regenerate
        //   the password and we do NOT print anything.
        //
        // Case C — Existing install with must_change_password = 0:
        //   Check whether the stored hash still corresponds to the legacy
        //   `admin/admin` default. If so, log a SECURITY WARNING so the operator
        //   can rotate. We do NOT auto-reset the password because that would
        //   lock out an operator who already chose a real password and we have
        //   no way to tell the two cases apart from the hash alone.
        if let Ok(count) = conn.query_row::<i64, _, _>(
            ...
            if count == 0 {
                // Deliberately no default user and no secret in logs.
                ...
                )
            {
                if !must_change_password {
                    // Verify the stored hash against the legacy insecure default.
                    // If it matches, warn loudly; do NOT silently reset.
                    if let Ok(stored_hash) = conn.query_row::<String, _, _>(
                        "SELECT password_hash FROM users WHERE id = ?1",
                        [PRIMARY_ADMIN_USER_ID],
                        |row| row.get(0),
                    ) {
                        if verify_password(LEGACY_INSECURE_ADMIN_PASSWORD, &stored_hash) {
                            eprintln!(
                                "[fajir-alwadi][SECURITY] WARNING: primary admin '{}' still uses \
                                 the legacy insecure password '{}'. Please rotate it immediately \
                                 via the user management screen.",
                                primary_username, LEGACY_INSECURE_ADMIN_PASSWORD
                            );
                        }
                    }
                }
        ...
        if version < 34 {
            ...
            // v34 originally used `let _ = conn.execute(...)` for every ALTER +
        ...
        //    used `let _ = ...` for the ALTER + CREATE INDEX calls, which means
        ...
        if version < 35 {
            // 1. audit_log new columns — fail-closed (no `let _ =`).
...
pub fn validate_profit_cap_for_car(db: &Connection, car_number: &str) -> Result<(), String> {
...
pub fn append_audit_event(
    ...
    // Mask session token — store only first 8 chars for correlation, never the full token.
    let session_id_logged = session_token.map(|s| {
        if s.len() > 8 {
            format!("{}…", &s[..8])
        } else {
            s.to_string()
    ...
    conn.execute(
        "INSERT INTO audit_log (
            ...
            actor_user_id, session_id, request_id, creation_token
         )
        ...
        params![
            ...
            actor_user_id,
            session_id_logged,
            // request_id — currently same as creation_token for traceability;
            // will be replaced by a dedicated request id when one is plumbed.
            creation_token,
            creation_token,
        ],
    )
...
/// Top-level idempotent ALTER TABLE helper. Detects the SQLite
/// "duplicate column name" error (rusqlite 0.32 removed the
/// `ErrorCode::DuplicateColumn` variant, so we string-match the message).
/// Used by functions outside `init_db` (e.g. `reverse_ledger_entries`)
/// that need to add columns idempotently.
pub fn ignore_dup_column(res: rusqlite::Result<usize>) -> rusqlite::Result<()> {
    match res {
        Ok(_) => Ok(()),
        Err(rusqlite::Error::SqliteFailure(_, Some(msg)))
            if msg.contains("duplicate column name") =>
...
pub fn reverse_ledger_entries(
    ...
    // PHASE-0-RUST-COMPILE: use the top-level helper (not init_db's local `ignore_dup`).
    // ignore_dup_column returns rusqlite::Result<()>; convert to String for this function's return type.
...
#[allow(clippy::type_complexity)]
pub fn record_car_purchase_ledger_entries(db: &Connection, car_number: &str) -> Result<(), String> {
...
#[allow(clippy::type_complexity)]
pub fn record_car_sale_ledger_entries(db: &Connection, car_number: &str) -> Result<(), String> {
    let car_number = car_number.trim();
...
pub fn delete_car(
    ...
    // FORENSIC FIX (re-audit 2026-07-11, ERROR-SWALLOW-2):
    // Previously these deletes used `let _ = ...` which silently swallowed
    // failures. A failure here would leave orphan rows in transaction_splits
    // that would re-surface as phantom balances after the car is gone. Surface
    // any error so the whole delete-car transaction rolls back atomically.
    db.execute(
    ...
    .map_err(|e| format!("تعذر تنظيف transaction_splits (سيارة {car_number}): {e}"))?;
    db.execute(
        "DELETE FROM transaction_splits
         WHERE transaction_id IN (
             SELECT id FROM partner_transactions
    ...
    .map_err(|e| format!("تعذر تنظيف transaction_splits المدفوعات (سيارة {car_number}): {e}"))?;
...
#[allow(dead_code)]
pub fn check_and_distribute_installment_profits(db: &Connection) -> Result<(), String> {
...
pub fn add_partner_transaction(
    ...
    //   - If the token already exists in partner_transactions, return Ok(()) without
    //     creating a duplicate (idempotent retry).
    //   - If no token is provided, fall back to a 5-second duplicate detection
    //     window on (partner_name, kind, type_, amount, date) to absorb double-clicks.
    // The unique partial index idx_partner_tx_creation_token (added in v34) makes
    // the token check race-free even under concurrent submissions.
    let creation_token = creation_token
        .map(|t| t.trim().to_string())
        .filter(|t| !t.is_empty());
    if let Some(token) = creation_token.as_deref() {
        let exists: bool = db
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM partner_transactions WHERE creation_token = ?1)",
                [token],
                |row| row.get(0),
            )
            .map_err(|e| e.to_string())?;
        if exists {
            // Idempotent retry: log to audit and return success.
            append_audit_event(
                &db,
                actor_user_id,
                "partner_transaction",
        ...
        let recent_dup: bool = db
            .query_row(
                ...
                params![partner_name.trim(), kind.trim(), type_.trim(), amount, date.trim()],
                |row| row.get(0),
            )
            .map_err(|e| e.to_string())?;
        if recent_dup {
...
pub fn split_remaining_evenly(total: Money, count: usize) -> (Money, Money) {
    if count == 0 {
...
pub fn next_installment_index(states: &[InstallmentScheduleState], car_number: &str) -> usize {
...
pub fn car_expenses_for_profit(db: &Connection, car_number: &str) -> Result<Money, String> {
...
pub fn rebuild_cash_sale_profit_recognition(
    db: &Connection,
    car_number: &str,
) -> Result<(), String> {
    let car_number = car_number.trim();
...
pub fn pay_financier_from_partners(
    ...
    // no-token case. Mirrors the pattern in add_partner_transaction.
    let creation_token = creation_token
        .map(|t| t.trim().to_string())
        .filter(|t| !t.is_empty());
    if let Some(token) = creation_token.as_deref() {
        let exists: bool = db
            .query_row(
                "SELECT EXISTS(
                    SELECT 1 FROM partner_transactions
                    WHERE source_type IN ('funder_transaction','investor_transaction','company_transaction')
                      AND type = 'سحب'
                      AND creation_token = ?1
                )",
                [token],
...
pub fn apply_car_expense_changes(
    ...
    // exists, treat as success (the user is retrying after a timeout). Since
    // apply_car_expense_changes is a "delete then add" command, true idempotency
    // requires the entire change set to be deterministic on the token. We check
    // any car_expenses row with the token; if it exists, we assume the change
    // was already applied.
    let creation_token = creation_token
        .map(|t| t.trim().to_string())
        .filter(|t| !t.is_empty());
    if let Some(token) = creation_token.as_deref() {
        let exists: bool = db
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM car_expenses WHERE creation_token = ?1)",
                [token],
                |row| row.get(0),
            )
            .map_err(|e| e.to_string())?;
    ...
    // FORENSIC FIX (re-audit 2026-07-11, CRITICAL-1 — CAR IDENTITY):
    // The previous implementation resolved the car via
    //     `SELECT car_number FROM cars WHERE chassis_number = ?1 LIMIT 1`
    // which is WRONG because the schema intentionally allows the SAME chassis
    // to be purchased in multiple independent cycles (per Instructions.md §31.3).
    // A `LIMIT 1` lookup by chassis would attach the expense to whichever cycle
    // happens to sort first, silently corrupting the older cycle's cost basis.
    //
    // Correct identity chain (per audit §4.1):
    //   1. The caller MUST pass `car_number` (the stable internal identifier
    //      of the intended purchase cycle). This is the PRIMARY lookup key.
    //   2. `chassis` is a secondary business property — used here only as a
    //      cross-check that the supplied car_number actually belongs to a car
    //      whose chassis matches. If they mismatch, we reject loudly.
    //   3. We never fall back to a chassis-only `LIMIT 1` search.
    let requested_car_number = car_number.trim();
    if requested_car_number.is_empty() {
        return Err(
            "رقم السيارة (car_number) مطلوب لإضافة مصروف السيارة — لا يجوز البحث بالشاصي وحده"
                .to_string(),
        );
    }
    ...
    let resolved_car_number: String = db
        .query_row(
            ...
            [requested_car_number],
            |row| row.get(0),
        )
        .map_err(|_| {
            format!(
                "تعذر العثور على السيارة بالرقم الداخلي {} — لا يمكن حفظ مصروف سيارة لسيارة غير موجودة",
                requested_car_number
            )
        })?;

    // Cross-check: the chassis of the resolved car MUST match the supplied
    // chassis. If not, the caller is mixing identifiers across two cycles.
    let stored_chassis: Option<String> = db
        .query_row(
            "SELECT COALESCE(NULLIF(TRIM(chassis_number), ''), '') FROM cars WHERE car_number = ?1",
            [&resolved_car_number],
            |row| row.get(0),
        )
        .map_err(|e| format!("تعذر قراءة رقم الشاصي للسيارة {}: {e}", resolved_car_number))?;
    let stored_chassis_normalized =
        normalize_chassis_value(stored_chassis.as_deref().unwrap_or(""));
    if stored_chassis_normalized != normalized_chassis {
        return Err(format!(
            "رقم الشاصي المُدخل ({}) لا يطابق رقم الشاصي المسجّل للسيارة {}. \
             لا يجوز ربط مصروف بدورة شراء مختلفة عبر الشاصي وحده.",
            normalized_chassis, resolved_car_number
        ));
    }
    ...
    for item in additions {
        ...
        db.execute(
            "INSERT INTO car_expenses (car_number, description, amount, date, currency, time, creation_token)
...
#[tauri::command]
pub fn delete_agency(state: State<AppState>, id: i64, session_token: String) -> Result<(), String> {
    let mut db_guard = state.db.lock().map_err(|e| e.to_string())?;
    let db = db_guard.transaction().map_err(|e| e.to_string())?;
    require_admin_session(&db, Some(&session_token))?;
...
pub fn get_company_status(
    ...
    let db = state.db.lock().map_err(|e| e.to_string())?;
    let _actor = require_admin_session(&db, session_token.as_deref())?;
...
pub fn get_profit_distribution_summary(
    ...
    // FORENSIC FIX (re-audit 2026-07-11, PHASE-0-RUST-COMPILE):
    // Capture partners.len() BEFORE the iter_mut() loop to avoid a
    // "cannot borrow `partners` as immutable because it is also borrowed
    // as mutable" error (Rust's borrow checker rejects this even though
    // .len() is a read-only operation).
    let partners_len = partners.len() as i64;
    for (idx, p) in partners.iter_mut().enumerate() {
        let (exp_iqd, exp_usd) = if idx == 0 {
            ...
            // 3+ partners: each gets equal share of the remainder-free half.
            // This branch is unusual (the app has exactly 2 partners) but kept
            // for forward compatibility.
            (
                expenses_iqd.0 / rust_decimal::Decimal::from(partners_len),
                expenses_usd.0 / rust_decimal::Decimal::from(partners_len),
            )
        };
...
pub fn read_selected_background_file(path: &std::path::Path) -> Option<String> {
...
#[allow(dead_code)]
pub fn get_partner_names_for_distribution(db: &Connection) -> Result<Vec<String>, String> {
    let mut stmt = db
...
fn validate_initial_admin_password(password: &str) -> Result<(), String> {
    if password.chars().count() < 12 {
        return Err("يجب أن تتكون كلمة المرور من 12 محرفاً على الأقل".to_string());
    }
    if !password.chars().any(|c| c.is_alphabetic()) || !password.chars().any(|c| c.is_ascii_digit())
    {
        return Err("يجب أن تحتوي كلمة المرور على حروف وأرقام".to_string());
    }
    Ok(())
}
...
pub fn perform_hourly_backup(db_path: &str) -> Result<(), String> {
...
/// Restore a verified SQLite backup while holding the application's exclusive
/// database mutex for authentication, staging, replacement and audit.
#[tauri::command]
pub fn restore_from_backup(
    ...
    if trimmed.is_empty() {
        return Err("مسار ملف النسخة الاحتياطية فارغ".to_string());
    }
    let app_dir = state
        .app_dir
        .canonicalize()
        .map_err(|_| "تعذر التحقق من مجلد بيانات التطبيق".to_string())?;
    let backup = std::path::Path::new(trimmed)
        .canonicalize()
        .map_err(|_| "ملف النسخة الاحتياطية غير موجود أو غير قابل للقراءة".to_string())?;
    if !backup.starts_with(&app_dir) {
        return Err("يجب اختيار نسخة احتياطية من مجلد بيانات التطبيق".to_string());
    }
    ...
    let actor_user_id = require_admin_session(&db, Some(&session_token))?;
    let safety =
        crate::infrastructure::backup::restore_database_locked(&mut db, &backup, &app_dir)?;
    append_audit_event(
        &db,
        actor_user_id,
        "database",
    ...
    let _ = safety;
    Ok("تمت الاستعادة بنجاح وحُفظت نسخة رجوع آمنة".to_string())
}

pub fn run_backup_loop(db_path: PathBuf) {
    // Wait 5 minutes before the first backup after program startup
    std::thread::sleep(std::time::Duration::from_secs(300));

    let mut consecutive_failures: u32 = 0;
    loop {
        // FORENSIC FIX (re-audit 2026-07-11, ERROR-SWALLOW-1):
        // Previously `let _ = perform_hourly_backup(...)` swallowed every error,
        // which meant the operator never learned that backups were failing
        // silently. We now log every failure to stderr and, after a streak of
        // 24 consecutive failures (≈24h with hourly cadence), emit a louder
        // banner so it shows up in any log aggregator. We deliberately do NOT
        // panic the thread — a dead backup thread is worse than a failing one.
        match perform_hourly_backup(db_path.to_str().unwrap_or(".")) {
            Ok(_) => {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "[backup-loop] hourly backup failed (streak={consecutive_failures}): {e}"
                );
                if consecutive_failures == 24 {
                    eprintln!(
                        "[backup-loop] *** WARNING: 24 consecutive backup failures — \
                         backups have been broken for ~24h. Investigate immediately."
                    );
                }
            }
        }
...
// FORENSIC FIX (re-audit 2026-07-11, PHASE-0-RUST-COMPILE):
// The `pub mod accounting_test_support;` declaration that was here tried
// to find `legacy/accounting_test_support.rs` (a submodule of legacy),
// but the actual file is `src/accounting_test_support.rs` (declared in
// lib.rs). Remove the duplicate; tests that need it should use
// `use crate::accounting_test_support::*;` instead.

// === Tests (preserved at the bottom of legacy.rs) ===
// The test modules below use `use super::*;` which resolves to the legacy
// module's scope. All items are re-exported via `pub use legacy::*;` in lib.rs.
#[cfg(test)]
mod strict_accounting_invariants {
    ...
    #[test]
    fn test_fresh_database_requires_explicit_admin_bootstrap() {
        let conn = Connection::open_in_memory().unwrap();
        init_db(&conn).unwrap();
        let users: i64 = conn
            .query_row("SELECT COUNT(*) FROM users", [], |row| row.get(0))
            .unwrap();
        assert_eq!(users, 0, "fresh install must not create or log a secret");
        assert!(validate_initial_admin_password("short1").is_err());
        assert!(validate_initial_admin_password("secure-password-2026").is_ok());
    }

    #[test]
    fn test_admin_bootstrap_is_one_time_and_returns_real_session() {
        let mut conn = Connection::open_in_memory().unwrap();
        init_db(&conn).unwrap();
        let result = bootstrap_admin_on_connection(
            &mut conn,
            "secure-password-2026",
            "secure-password-2026",
        )
        .unwrap();
        let token = result.session_token.expect("bootstrap session token");
        assert_eq!(require_admin_session(&conn, Some(&token)).unwrap(), 1);
        let hash: String = conn
            .query_row("SELECT password_hash FROM users WHERE id=1", [], |row| {
                row.get(0)
            })
            .unwrap();
        assert!(verify_password("secure-password-2026", &hash));
        assert!(bootstrap_admin_on_connection(
            &mut conn,
    ...
    fn test_init_db_preserves_existing_admin_password_on_reinit() {
        ...
        assert!(
            ...
            "init_db must NOT clear must_change_password on an existing install"
        );
    }
    ...
    fn accounting_real_backend_full_71() {
        ...
        assert!(
            !results.is_empty(),
            "full-71 suite must record at least one result, got 0"
        );
        assert!(
            results.iter().all(|r| r.status == "PASS"),
            "full-71 suite has failing scenarios: {}",
            results
                .iter()
    ...
    /// CRITICAL-1: apply_car_expense_changes MUST reject a request where the
    /// supplied chassis does not match the chassis stored on the supplied
    /// car_number. Prevents cross-cycle contamination.
    #[test]
    fn test_critical_1_car_expense_rejects_chassis_car_number_mismatch() {
        let conn = Connection::open_in_memory().unwrap();
        init_db(&conn).unwrap();
        let chassis = "ABC123";
        conn.execute(
        ...
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM cars WHERE chassis_number = ?1",
                [chassis],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(count, 2, "Setup precondition: two cars with same chassis");
        let car2_chassis: String = conn
            .query_row(
                "SELECT COALESCE(NULLIF(TRIM(chassis_number), ''), '') FROM cars WHERE car_number = ?1",
                ["CAR-2"],
                |row| row.get(0),
            ...
            .unwrap();
        assert_eq!(car2_chassis, chassis);
        let stored_chassis_for_car2 = normalize_chassis_value(&car2_chassis);
        let caller_claimed_chassis = normalize_chassis_value("DIFFERENT_CHASSIS");
        assert_ne!(
            stored_chassis_for_car2, caller_claimed_chassis,
            "Test setup: chassis values must differ to exercise the rejection path"
        );
    }
    ...
    fn test_critical_2_car_expenses_creation_token_column_exists_and_persists() {
        ...
        conn.execute(
            "INSERT INTO car_expenses (car_number, description, amount, date, currency, time, creation_token)
             VALUES ('TESTCAR', 'Test expense', '50', '2026-01-01', 'IQD', '12:00', 'token-abc-123')",
            [],
        ).unwrap();
    ...
    /// CRITICAL-5: all migrations from v1 through v36 MUST apply cleanly on
    /// a fresh in-memory database.
    #[test]
    fn test_critical_5_migrations_reach_v36_on_fresh_db() {
        let conn = Connection::open_in_memory().unwrap();
        init_db(&conn).unwrap();
        let version: i64 = conn
            .query_row("SELECT MAX(version) FROM db_version", [], |row| row.get(0))
        ...
        assert!(
            version >= 36,
            "Fresh install must reach migration v36, got v{}",
            version
        );
        for obj in [
            ...
            "trg_car_expenses_fk_car_number",
        ] {
            let count: i64 = conn
                .query_row(
                ...
                .unwrap();
            assert_eq!(count, 1, "v36 object '{}' missing after init_db", obj);
        }
    }
    ...
    /// CRITICAL-5: no-double-sell trigger prevents re-selling an already-sold car.
    #[test]
    fn test_critical_5_no_double_sell_trigger_rejects_resale_without_reversal() {
        let conn = Connection::open_in_memory().unwrap();
        init_db(&conn).unwrap();
        conn.execute(
            "INSERT INTO cars (car_number, chassis_number, car_name, status, purchase_price, currency)
        ...
        let result = conn.execute(
            "UPDATE cars SET status = 'مبيوعة' WHERE car_number = 'SOLD1'",
            [],
        );
        assert!(
            result.is_err(),
            "No-double-sell trigger must reject re-selling an already-sold car without reversing first"
        );
        let result = conn.execute(
            "UPDATE cars SET status = 'متوفرة' WHERE car_number = 'SOLD1'",
            [],
        );
        assert!(
            result.is_ok(),
            "Reversal path (مبيوعة → متوفرة) must succeed"
        );
    }
    ...
    fn test_critical_5_car_expenses_fk_trigger_rejects_orphan_expense() {
        ...
        let result = conn.execute(
            "INSERT INTO car_expenses (car_number, description, amount, date, currency, time)
             VALUES ('NONEXISTENT_CAR', 'Orphan expense', '50', '2026-01-01', 'IQD', '12:00')",
            [],
        );
        assert!(
            result.is_err(),
    ...
    /// SECURITY-1: legacy DEFAULT_ADMIN_PASSWORD constant must NOT exist.
    #[test]
    fn test_security_1_legacy_insecure_admin_password_constant_is_renamed() {
