...
function mockStorageKey(command: string): string {
  ...
  if (command.includes("agency_transaction")) return "mock_agency_transactions";
  if (command.includes("agency")) return "mock_agencies";
  return "mock_default";
}
...
async function mockInvoke<T>(
  ...
  if (command === "add_car") {
    ...
    const normalizedChassis = normalizeVehicleIdentifier(args.chassis);
    if (!normalizedChassis) {
      throw new Error("رقم الشاصي مطلوب");
    }
    ...
    // We no longer reject duplicates — each car gets its own car_number.
    ...
    const item = {
      ...mapMockCar({ ...args, num: carNumber, chassis: normalizedChassis }),
      car_number: carNumber,
      car_plate_num: plateNum,
  ...
  if (command === "apply_car_expense_changes") {
    const cars: Car[] = JSON.parse(localStorage.getItem("mock_cars") ?? "[]");
    const chassis = normalizeVehicleIdentifier(args.chassis);
    const car = cars.find((item) => normalizeVehicleIdentifier(item.chassis_number) === chassis);
    if (!car) throw new Error("تعذر العثور على السيارة بعد حفظها");

    const deleteIds = new Set<number>((args.deleteIds ?? args.delete_ids ?? []).map((id: unknown) => Number(id)));
    const additions = Array.isArray(args.additions) ? args.additions : [];
    const existing: CarExpenseRecord[] = JSON.parse(localStorage.getItem("mock_car_expenses") ?? "[]");

    for (const id of deleteIds) {
      const record = existing.find((item) => item.id === id);
      if (!record || record.car_number !== car.car_number) {
        throw new Error(`المصروف رقم ${id} لا يخص السيارة المحددة`);
      }
    }
    for (const item of additions) {
      if (!String(item.description ?? "").trim() || compareMoney(item.amount, 0) <= 0) {
        throw new Error("بيانات مصروف السيارة غير صالحة");
      }
    }

    const next = existing.filter((item) => !deleteIds.has(item.id));
    for (const item of additions) {
      next.push({
        id: generateMockTxId(),
    ...
    localStorage.setItem("mock_car_expenses", JSON.stringify(next));
    return undefined as T;
  }
...
if (command === "get_profit_distribution_summary") {
  ...
  if (command === "get_backgrounds") {
    return [
      "/backgrounds/3D_fluid_shapes_dark_UI_202606271858.jpeg",
      "/backgrounds/Abstract_automotive_luxury_backg_U2026_202606271858.jpeg",
      "/backgrounds/Abstract_background_dark_theme_s_U2026_202606271858.jpeg",
      "/backgrounds/Dark_abstract_wallpaper_glass_wi_U2026_202606271858.jpeg"
    ] as unknown as T;
  }

  if (command === "get_selected_background") {
    return (localStorage.getItem("app_selected_background") || "/backgrounds/3D_fluid_shapes_dark_UI_202606271858.jpeg") as unknown as T;
  }

  if (command === "set_selected_background") {
    const background = String(args.background ?? "/backgrounds/3D_fluid_shapes_dark_UI_202606271858.jpeg");
    localStorage.setItem("app_selected_background", background);
    return background as unknown as T;
  }

  if (command === "rename_background") {
    return "/backgrounds/3D_fluid_shapes_dark_UI_202606271858.jpeg" as unknown as T;
  }
  ...
  if (command === "save_and_sell_car_with_accounting") {
    ...
    const item = {
      ...mapMockCar({ ...args, num: carNumber, status: "مبيوعة" }),
      car_number: carNumber,
      car_plate_num: plateNum,
